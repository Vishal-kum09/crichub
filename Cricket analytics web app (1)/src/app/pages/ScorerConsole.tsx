...
export function ScorerConsole({ matchId, onNavigate }: ScorerConsoleProps) {
  ...
  const [currentBowlingEnd, setCurrentBowlingEnd] = useState('');
  const [showEndSelectionDialog, setShowEndSelectionDialog] = useState(false);
  ...
  const [wickets, setWickets] = useState(() => {
    const raw = sessionStorage.getItem(SCORER_SESSION_KEY);
    return raw ? JSON.parse(raw).wickets ?? 0 : 0;
  });
  const [overs, setOvers] = useState(() => {
    const raw = sessionStorage.getItem(SCORER_SESSION_KEY);
    return raw ? JSON.parse(raw).overs ?? 0 : 0;
  });
  const [balls, setBalls] = useState(() => {
    const raw = sessionStorage.getItem(SCORER_SESSION_KEY);
    return raw ? JSON.parse(raw).balls ?? 0 : 0;
  });
  ...
  const [striker, setStriker] = useState(() => {
    const sess = getLiveSession();
    return sess?.playerNames?.[sess?.strikerId || ''] || 'Striker 1';
  });
  const [nonStriker, setNonStriker] = useState(() => {
    const sess = getLiveSession();
    return sess?.playerNames?.[sess?.nonStrikerId || ''] || 'Non-Striker';
  });
  const [currentBowler, setCurrentBowler] = useState(() => {
    const sess = getLiveSession();
    return sess?.playerNames?.[sess?.bowlerId || ''] || 'Opening Bowler';
  });
  ...
  const [batsmen, setBatsmen] = useState<Batsman[]>(() => {
    const sess = getLiveSession();
    return [
      { name: sess?.playerNames?.[sess?.strikerId || ''] || 'Striker 1', runs: 0, balls: 0, fours: 0, sixes: 0, isOut: false },
      { name: sess?.playerNames?.[sess?.nonStrikerId || ''] || 'Non-Striker', runs: 0, balls: 0, fours: 0, sixes: 0, isOut: false }
    ];
  });
  ...
  const [bowlers, setBowlers] = useState<Bowler[]>(() => {
    const sess = getLiveSession();
    return [
      { name: sess?.playerNames?.[sess?.bowlerId || ''] || 'Opening Bowler', overs: 0, balls: 0, runs: 0, wickets: 0, maidens: 0 }
    ];
  });
  ...
  const [teamNames, setTeamNames] = useState<any | null>(null);
  ...
  const activeFieldingRoster = session?.fieldingRoster?.length ? session.fieldingRoster : fieldingRoster;
  ...
  const refreshViewerScorecard = () => {
    ...
    fetchViewerScorecard(matchId)
      .then((data) => {
        ...
        setScorecardRefreshKey(prevKey => prevKey + 1);
      })
  ...
  useEffect(() => {
    ...
    getMatchPreview(matchId)
      .then((preview: ApiMatchPreview) => {
        ...
        const p = preview as MatchPreview;
        if (p.near_end) setNearEnd(p.near_end);
        if (p.far_end) setFarEnd(p.far_end);
      })
      .catch(() => {});
  }, [matchId]);

  useEffect(() => {
    if (isLive && !currentBowlingEnd && matchId && !inningsBreak) {
      setShowEndSelectionDialog(true);
    }
  }, [isLive, currentBowlingEnd, matchId, inningsBreak]);
  ...
  const nameForPlayer = (playerObj?: any, namesMap?: any) => 
    playerObj?.player_name || namesMap?.[playerObj?.player_id] || session?.playerNames?.[playerObj?.player_id] || 'Unknown Player';

  const hydrateFromLiveState = (liveState: LiveMatchState) => {
    if (!liveState?.ok || !liveState.innings || !matchId) return;

    const strikerObj = liveState.striker;
    const nonStrikerObj = liveState.non_striker || (liveState as any).nonStriker;
    const bowlerObj = liveState.bowler;
    
    const getRuns = (obj: any) => obj?.runs ?? obj?.runs_scored ?? obj?.runsScored ?? 0;
    const getBalls = (obj: any) => obj?.balls ?? obj?.balls_faced ?? obj?.ballsFaced ?? 0;
    const getFours = (obj: any) => obj?.fours ?? 0;
    const getSixes = (obj: any) => obj?.sixes ?? 0;
    const getIsOut = (obj: any) => obj?.is_dismissed ?? false;

    const playerNames = liveState.playerNames || {};
    const nextSession: LiveSession = {
      matchId,
      inningsId: liveState.innings.innings_id,
      strikerId: strikerObj?.player_id,
      nonStrikerId: nonStrikerObj?.player_id,
      bowlerId: bowlerObj?.player_id,
      playerNames,
    ...
    const strikerName = nameForPlayer(strikerObj, playerNames);
    const nonStrikerName = nameForPlayer(nonStrikerObj, playerNames);
    const bowlerName = nameForPlayer(bowlerObj, playerNames);
    ...
    if (liveState.battingRoster && liveState.battingRoster.length <= 12) setBattingRoster(liveState.battingRoster);
    if (liveState.fieldingRoster && liveState.fieldingRoster.length <= 12) setFieldingRoster(liveState.fieldingRoster);
    ...
    if (liveState.recent_deliveries && liveState.recent_deliveries.length > 1) {
      const lastDelivery = liveState.recent_deliveries[0];
      const lastOver = lastDelivery.over_number;
      const prevOverDelivery = liveState.recent_deliveries.find(d => d.over_number < lastOver);
      if (prevOverDelivery) {
        setPreviousBowler(nameForPlayer({ player_id: prevOverDelivery.bowler_id }, playerNames));
      }
    }
    ...
    setBatsmen([
      {
        name: strikerName,
        runs: getRuns(strikerObj),
        balls: getBalls(strikerObj),
        fours: getFours(strikerObj),
        sixes: getSixes(strikerObj),
        isOut: getIsOut(strikerObj),
      },
      {
        name: nonStrikerName,
        runs: getRuns(nonStrikerObj),
        balls: getBalls(nonStrikerObj),
        fours: getFours(nonStrikerObj),
        sixes: getSixes(nonStrikerObj),
        isOut: getIsOut(nonStrikerObj),
      },
    ...(liveState.batting_order || [])
      .filter(
        (p: any) =>
          p.player_id !== strikerObj?.player_id &&
          p.player_id !== nonStrikerObj?.player_id
      )
      .map((p: any) => ({ ...p, name: playerNames[p.player_id] || 'Unknown', isOut: p.is_dismissed }))
    ].filter((b: any) => b.name !== 'Unknown Player'));

    
    if (bowlerObj) {
      const bowlerBalls = bowlerObj.balls_bowled ?? 0;
      const allBowlers = (liveState.bowling_order || []).map((b: any) => ({
        ...b,
        name: nameForPlayer(b, playerNames),
        overs: Math.floor(b.balls_bowled/6),
        balls: b.balls_bowled
      }));
      if (!allBowlers.some((b: any) => b.name === bowlerName)) {
        allBowlers.push({
          name: bowlerName,
          overs: Math.floor(bowlerBalls / 6),
          balls: bowlerBalls,
          runs: bowlerObj.runs_conceded ?? 0,
          wickets: bowlerObj.wickets ?? 0,
          maidens: bowlerObj.maidens ?? 0,
        });
      }
    ...
    try {
      ...
      if (raw) {
        ...
        if (saved && saved.matchId === matchId) {
          if (saved.currentBowlingEnd) setCurrentBowlingEnd(saved.currentBowlingEnd);
          if (saved.previousBowler) setPreviousBowler(saved.previousBowler);
  ...
  // 🟢 FIXED: REMOVED `!session?.inningsId` BLOCKER SO IT ALWAYS FETCHES TRUE BACKEND STATE
  useEffect(() => {
    if (!matchId) return;
  ...
  // removed session?.inningsId dependency
  ...
  const recordBall = (wicketOverride?: {
    ...
    if (isConnected && session) {
      ...
      const extraRuns =
        ...
        : extraType === 'NB' ? Math.max(0, currentExtras - scoringRules.no_ball_penalty_runs)
  ...
  const advanceCreaseAfterDelivery = (
    ...
    if (isLegal) {
      if (balls === 5) {
        ...
        setCurrentBowlingEnd(prev => prev === nearEnd ? farEnd : nearEnd); 
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden bg-gray-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 items-start px-4 pb-2 border-b border-gray-200 shrink-0">
        ...
        {/* Column 2: Batsmen */}
        <div className="bg-white rounded-lg shadow-sm p-1.5 border border-gray-200 flex flex-col justify-between h-[122px]">
          <div>
            <p className="text-[10px] text-gray-500 mb-0.5 uppercase tracking-wide font-bold px-1">Batsman</p>
            <div className="space-y-1.5">
              <div className="flex items-center justify-between p-1 bg-green-50 rounded-md border border-green-200">
                <div>
                  <span className="font-bold text-xs text-gray-900">{striker}*</span>
                  <p className="text-[9px] text-gray-500 font-semibold">On Strike</p>  
                </div>
              ...
              <div className="flex items-center justify-between p-1 bg-gray-50 rounded-md">
                <div>
                  <span className="font-bold text-xs text-gray-900">{nonStriker}</span>
                  <p className="text-[9px] text-gray-500 font-semibold">Non-Striker</p>
                </div>
                <span className="text-xs font-bold tabular-nums">
          ...
          <button
            onClick={swapStrike}
            disabled={!isLive}
            className="w-full mt-1.5 px-2 py-1 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 transition-colors text-[10px] font-bold"
          >
            Change Strike
          </button>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-1.5 border border-gray-200 flex flex-col justify-between h-[122px]">
          <div className="space-y-1">
            <div>
              <p className="text-[10px] text-gray-500 mb-0.5 uppercase tracking-wide font-bold px-1">Current Bowler</p>
              <div className="flex items-center justify-between p-1 bg-blue-50 rounded-md border border-blue-200">
                <div>
                  <span className="font-semibold text-xs text-[#1a1a1a]">{currentBowler}</span>
                  <p className="text-[9px] text-[#666666]">End: {currentBowlingEnd || 'Select End'}</p>
                </div>
                <span className="text-xs font-bold tabular-nums">
            ...
            <div>
              <p className="text-[10px] text-gray-500 mb-0.5 uppercase tracking-wide font-bold px-1">Previous Bowler</p>
              {previousBowler ? (
                <div className="flex items-center justify-between p-1 bg-gray-50 rounded-md">
                  <div>
                    <span className="font-semibold text-xs text-[#1a1a1a]">{previousBowler}</span>
                    <p className="text-[9px] text-[#666666]">Last Over</p>
                  </div>
                ...
                <div className="flex items-center justify-center h-[26px] p-0.5 bg-gray-50 rounded-md border border-dashed border-gray-200">
                  <span className="text-[9px] font-medium text-gray-400">No previous bowler</span>
                </div>
              )}
            </div>
          </div>
          ...
          <button
            onClick={() => setShowBowlerChangeDialog(true)}
            disabled={!isLive}
            className="w-full mt-1.5 px-2 py-1 bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 disabled:opacity-50 transition-colors text-[10px] font-bold"
          >
            Change Bowler
          </button>
        </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-2 border border-gray-200 shrink-0">
              ...
              <div>
                ...
                <div className="grid grid-cols-7 gap-y-3 mt-4">
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-xl font-medium bg-gray-200 text-gray-800 hover:bg-gray-300 disabled:opacity-50 transition-all flex items-center justify-center gap-1 text-xs"
                  >
                    <Undo size={16} /> Undo
                  </button>
                  <button
              ...
              <div className="flex gap-2 mt-4">
                <button
                  ...
                  disabled={!isLive}
                  className="flex-1 p-3 bg-red-100 text-red-800 rounded-xl font-semibold text-base hover:bg-red-200 disabled:opacity-50 transition-all"
                >
                ...
                <button
                  onClick={() => recordBall()}
                  disabled={!isLive}
                  className="flex-1 p-3 bg-green-600 text-white rounded-xl font-bold text-base hover:bg-green-700 disabled:opacity-50 transition-all shadow-lg flex items-center justify-center gap-2"
                >
                  ✓ RECORD BALL
                </button>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-3 border border-gray-200 shrink-0">
              ...
              <div className="mt-2 flex rounded-md shadow-sm">
                <input
                  ...
                  id="event-logger"
                  name="event"
                  className="flex-1 block w-full rounded-none rounded-l-md sm:text-sm border-gray-300 disabled:bg-gray-100"
          ...
          <div className="flex items-center justify-between shrink-0 mb-2 p-2 bg-gray-100 dark:bg-gray-900 rounded-lg">
            <div className="flex gap-1 p-1 bg-gray-200 dark:bg-gray-800 rounded-md">
              ...
              <button
                ...
                disabled={!viewerScorecard || viewerScorecard.innings.length < 2}
      ...
      {/* Bowling End Selection Popup */}
      {showEndSelectionDialog && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm bg-white rounded-2xl shadow-2xl z-50 p-6 text-center">
            <h3 className="text-xl font-bold mb-2">Select Bowling End</h3>
            <p className="text-sm text-gray-500 mb-6">Which end is the bowler starting from?</p>
            <div className="grid grid-cols-1 gap-3">
              <button 
                onClick={() => { setCurrentBowlingEnd(nearEnd); setShowEndSelectionDialog(false); }} 
                className="p-4 bg-gray-50 border border-gray-200 rounded-xl font-bold hover:bg-[#e60023] hover:text-white transition-colors"
              >
                {nearEnd}
              </button>
              <button 
                onClick={() => { setCurrentBowlingEnd(farEnd); setShowEndSelectionDialog(false); }} 
                className="p-4 bg-gray-50 border border-gray-200 rounded-xl font-bold hover:bg-[#e60023] hover:text-white transition-colors"
              >
                {farEnd}
              </button>
      ...
      {showBowlerChangeDialog && (
        <>
          <div className="fixed inset-0 bg-black/50 z-40" />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl shadow-2xl z-50 p-6">
            <h3 className="text-xl font-semibold mb-4">Change Bowler</h3>
            <p className="text-sm text-[#666666] mb-4">Select a bowler from the active Playing XI roster.</p>
            {/* 🟢 FIXED: Active Fielding Roster bounds used here */}
            <select
